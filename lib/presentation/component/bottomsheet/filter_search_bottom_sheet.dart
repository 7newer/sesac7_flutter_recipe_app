...
class FilterSearchBottomSheet extends StatefulWidget {
  ...
  @override
  State<FilterSearchBottomSheet> createState() => _FilterSearchBottomSheetContentState();
}

class _FilterSearchBottomSheetContentState extends State<FilterSearchBottomSheet> {
  // 선택된 필터 값을 저장할 변수들
  String _selectedTime = 'Newest'; // 기본값 'Newest'
  int _selectedRate = 4; // 기본값 4점
  List<String> _selectedCategories = ['Local Dish']; // 기본값 'Local Dish'
    ...
    return SingleChildScrollView(
      child: Container(
        ...
        decoration: const BoxDecoration(
          ...
          borderRadius: BorderRadius.vertical(top: Radius.circular(25.0)), // 상단 모서리 둥글게
        ),
        child: Column(
  ...
  Widget _buildFilterButtonWidget({
    ...
    return Padding(
      ...
      child: Material( // 터치 피드백을 위해 Material 위젯 사용
        color: Colors.transparent, // Material의 기본 색상 제거
        child: InkWell( // 터치 피드백을 위해 InkWell 사용
          onTap: onPressed,
          borderRadius: BorderRadius.circular(30.0),
          child: Container(
            padding: const EdgeInsets.symmetric(horizontal: 16.0, vertical: 8.0),
            decoration: BoxDecoration(
              color: isSelected ? const Color(0xFF2196F3).withOpacity(0.8) : const Color(0xFFE0F2F1), // 이미지 색상에 맞게 조정
              borderRadius: BorderRadius.circular(30.0),
              border: Border.all(
                color: isSelected ? Colors.transparent : Colors.grey.shade300, // 선택 안되면 회색 테두리
              ),
            ),
            child: Row(
              ...
              children: [
                Text(
                  text,
                  style: TextStyle(
                    color: isSelected ? Colors.white : Colors.black87, // 선택 여부에 따라 텍스트 색상 변경
                    fontWeight: isSelected ? FontWeight.bold : FontWeight.normal,
                  ),
                ),
                if (icon != null) ...[ // 아이콘이 있으면 아이콘 추가
  ...
  Widget _buildCategoryFilterButtons() {
    final List<String> categoryOptions = [
      ...
      'Local Dish', 'Fruit', 'Breakfast', 'Spanish', 'Chinese', 'Lunch'
    ];
    return Wrap( // Wrap 위젯을 사용하여 자동 줄바꿈
      spacing: 8.0, // 가로 간격
      runSpacing: 8.0, // 세로 간격
      children: categoryOptions.map((category) {
        return _buildFilterButtonWidget(
          text: category,
          isSelected: _selectedCategories.contains(category), // List에 포함되어 있는지 확인
          onPressed: () {
            setState(() {
              if (category == 'All') { // 'All'을 선택하면 다른 모든 선택 해제
                _selectedCategories = ['All'];
              } else if (_selectedCategories.contains('All') && category != 'All') {
                ...
                if (_selectedCategories.isEmpty) { // 아무것도 선택 안되면 'All' 선택
    ...
    return SizedBox( // 가로 전체를 차지하도록
      ...
      child: ElevatedButton(
        ...
        style: ElevatedButton.styleFrom(
          backgroundColor: const Color(0xFF2196F3).withOpacity(0.8), // 버튼 배경색
